...
def prove(candidate_id: str, repo: Path, backend: str, write_test_dir: bool, force: bool, run: bool) -> None:
    ...
    generator = proof[0] if proof is not None else None
    targets = proof[1] if proof is not None else None
    ...
    if generator is not None and targets is not None:
        candidate.evidence.notes.append(
            f"Executable PoC generated against {targets.describe()}."
        )
        ...
        runnable_source = generator.render_poc(
            candidate, targets, poc_location="test/zeropath"
        )
    ...
    if write_test_dir and generator is None:
        try:
    ...
    if run and backend == "foundry":
        verbosity = generator.verbosity if generator is not None else 0
        ...
        if generator is not None:
            ...
            generator.interpret(candidate, result, measured)
